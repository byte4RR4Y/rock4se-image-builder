...
make -j ${CPUS} KERNELRELEASE="${BUILD}" ARCH=arm64 CONFIG_BRCMUTIL=m CONFIG_BRCMFMAC=m CONFIG_ARCH_ROCKCHIP=y CONFIG_WIRELESS=y CONFIG_DRM_VIRTIO_GPU=y DRM_VIRTIO_GPU_KMS=y CONFIG_DRM_PANEL_SITRONIX_ST7703=y CONFIG_DRM_PANEL_BOE_TV101WUM_NL6=y CONFIG_DRM_PANEL_LVDS=y CONFIG_DRM_PANEL_SIMPLE=y CONFIG_DRM_PANEL_EDP=y CONFIG_DRM_PANEL_ILITEK_ILI9882T=y CONFIG_DRM_PANEL_MANTIX_MLAF057WE51=y CONFIG_DRM=y CROSS_COMPILE=aarch64-linux-gnu- Image.gz modules dtbs
